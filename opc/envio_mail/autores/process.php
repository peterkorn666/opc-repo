...
foreach($ids_personas as $id_autor){
    ...
    if($rowTrabajo["archivo_tl"]){
        $archivo_tl = "<a href='".$config["url_opc"]."tl/".$rowTrabajo["archivo_tl"]."'>Descargue aquí el TRABAJO COMPLETO.</a>";
    } else {
        $archivo_tl = "";
    }
	...
	$cuerpoMail = str_replace("<:nombreContacto>", utf8_decode($row["contacto_nombre"]), $cuerpoMail);
	$cuerpoMail = str_replace("<:apellidoContacto>", utf8_decode($row["contacto_apellido"]) , $cuerpoMail);
	$cuerpoMail = str_replace("<:clave>", $rowTrabajo["clave"] , $cuerpoMail);
    $cuerpoMail = str_replace("<:titulo_tl>", utf8_decode($rowTrabajo["titulo_tl"]), $cuerpoMail);
	$cuerpoMail = str_replace("<:mail>", $row["contacto_mail"] , $cuerpoMail);
	...
	$cuerpoMail = str_replace("<:comentarios_evaluador>", utf8_decode($comentarios_evaluador) , $cuerpoMail);
	$cuerpoMail = str_replace("<:presentador_tl>", utf8_decode($presentador_trabajo) , $cuerpoMail);
	...
	//agregado para los certificos
	$autorparacertifico ="<a href='".$config["url_opc"]."acp/certificado_autor.php?t='".base64_encode($rowTrabajo["id_trabajo"])."'&k='".base64_encode($row["ID_Personas"]).">Constancia de ponencia</a>";	
	$cuerpoMail = str_replace("<:link_certificaado_autor>",$autorparacertifico, $cuerpoMail);
	//fin del agrego para el certificos
