...
foreach($_POST["trabajo"] as $id_trabajo){
    ...
    //Archivo TL
    if($row["archivo_tl"]){
        $archivo_tl = "<a href='".$config["url_opc"]."tl/".$row["archivo_tl"]."'>Descargue aqu&iacute; el TRABAJO COMPLETO para su evaluaci&oacute;n.</a>";
    } else {
        $archivo_tl = "";
    }
    ...
    $cuerpoMail = str_replace("<:titulo_tl>", utf8_decode($row["titulo_tl"]) , $cuerpoMail);
    $cuerpoMail = str_replace("<:autores_tl>", utf8_decode($autoresTL) , $cuerpoMail);
