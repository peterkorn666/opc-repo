...
include("../clases/Evaluador.php");

$cartas_instancia = new Cartas();
$evaluador_instancia = new Evaluador();

$config = $cartas_instancia->getConfig();
$cartas_predefinidas = $cartas_instancia->getCartasEvaluadores();

if(count($_POST["evaluadores"]) > 0){
    $evaluadores_mail = $evaluador_instancia->getEvaluadoresMailByIDS($_POST["evaluadores"]);
} else {
    $evaluadores_mail = $evaluador_instancia->getEvaluadoresMail();
}
...
<head>
    ...
    <title>Evaluadores - Envio de mail - <?=$config["nombre_congreso"]?></title>
...
<body>
    <div class="container">
        ...
        <form id="formCartaEvaluadores" action="process.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="col-md-12 datos_envio">
                <div class="row">
                    <div class="col-md-8">
                        <input type="checkbox" name="enviar_a_destinatario_especifico" value="1">  Enviar mail de cada evaluador seleccionado a un solo destinatario, el cual es:<br>
                        <input type="text" name="destinatario_especifico" class="form-control">
                    </div>
                </div><br>
                <div class="row">
                    <div class="col-md-8">
                        <input type="checkbox" name="enviar_a_personas" value="1"> Enviar el mail correspondiente al evaluador seleccionado
                    </div>
                </div><br>
                <div class="row">
                    <div class="col-md-12">
            ...
            <!-- end Recuadro 2 -->
            <br>
            <!-- Recuadro 3 - Datos de los evaluadores -->
            <div class="col-md-12 datos_listado">
                <div class="row div_info">
                    <div class="col-md-12">
                        <strong>Evaluadores</strong><br>
                        Hay [<strong><?=count($evaluadores_mail)?></strong>] evaluadores.
                    </div>
                </div>
                <div class="row div_filtros">
                    <div class="col-md-6">
                        <label>Evaluadores con evaluaciones:</label>
                        <select name="filtro_evaluaciones" class="form-control">
                            <option value=""></option>
                            <option value="1">Por lo menos 1</option>
                        </select>
                    </div>
                    <div class="col-md-6">
                ...
                <div id="listado">
                <?php
                foreach($evaluadores_mail as $evaluador){
                    $evaluaciones = $evaluador_instancia->getEvaluacionesByEvaluador($evaluador["id"]);
                    $cont_evaluaciones = 0;
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <input type="checkbox" id="personas[]" name="personas[]" value="<?=$evaluador["id"]?>" class="checkbox_evaluadores" data-numero_evaluaciones="<?=$cont_evaluaciones?>" data-email="<?=$evaluador["mail"]?>"> <?=$evaluador["nombre"]?> - (<?=$evaluador["mail"]?>)
                        </div>
                    </div>
                    <?php
                }
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#formCartaEvaluadores").submit(function(){
                    ...
                    "<div class='col-md-12'>" +
                        "<div class='row'>" +
                            "<div class='col-md-12'>" +
                                ...
                                "<input type='checkbox' name='asunto_nombre' value='1'> Agregar nombre - <span class='asunto_muestra'></span> [Nombre]" +
                            "</div>" +
                        "</div><br>" +
                        "<div class='row'>" +
                            "<div class='col-md-12'>" +
                    ...
                    "<div class='col-md-12'>" +
                        "<div class='row'>" +
                            "<div class='col-md-12'>" +
                                ...
                                "<input type='checkbox' name='asunto_nombre' value='1'> Agregar nombre - "+asunto+" [Nombre]" +
                            "</div>" +
                        "</div>" +
                    "</div>");
    ...
    function marcarTodos(){
        $(".checkbox_evaluadores").each(function(){
            if($(this).prop('disabled') === false){
                var se_marca = true;
                var filtro_evaluaciones = $("select[name='filtro_evaluaciones'] option:selected");
                if(filtro_evaluaciones.val() !== ""){

                    var numero_evaluaciones = $(this).data('numero_evaluaciones');
                    if(parseInt(numero_evaluaciones) < parseInt(filtro_evaluaciones.val())){
                        se_marca = false;
                    }
                }
                /*if(se_marca === true){
    ...
    function testEmailsEvaluadores(){
        $(".checkbox_evaluadores").each(function(){
