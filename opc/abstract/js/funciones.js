...
jQuery(document).ready(function($) {
	...
	$(document).on("click","#step1",function(e){
		...
		/*if(parseInt($("[name='words_total']").val())>1500)
		{
			alert("El total de palabras no puede ser mayor a 1500");
			return false;
		}*/
...
function setTotalWords(){
	setTimeout(function(){
		...
		var limite;
		var count1;
		if($('input:radio[name=tipo_conversatorio]:checked').val()==2){
			limite=3000;
		}else{
			limite=1500;
		}
		...
		var count;
		$(".cke_wordcount span").each(function(index, element) {
			...
			if(jQuery(element).html())//index!=0
			{
				count1 = parseInt(jQuery(element).html().split(" ")[3]);
				if (count1 > totalWords1){
					totalWords1 = count1;
				}
			}
		});	
		var type = (totalWords>limite ? 'red' : 'green');
		var type1 = (totalWords1>limite ? 'red' : 'green');
		//type = 'green';
		jQuery("#totalwords").html("<b style='color:"+type+"'>"+totalWords+"</b><b>/"+limite+"</b>");
		jQuery("#totalwords1").html("<b style='color:"+type1+"'>"+totalWords1+"</b><b>/"+limite+"</b>");
		jQuery("input[name='totalwords']").val(totalWords);
		jQuery("input[name='totalwords1']").val(totalWords1);
	},0);
}
...
function tieneEje(){
	...
	var div_tiene_descripcion = jQuery(".div-tiene_descripcion");
	var div_no_tiene_descripcion = jQuery(".div-no_tiene_descripcion");
	...
	if (modalidad.val() === undefined || modalidad.val() === null || modalidad.val() === ""){
		...
		div_tiene_eje.show();
		div_no_tiene_descripcion.show();
		div_tiene_descripcion.hide();
	} else {
		...
		var tiene_descripcion = modalidad.data('tiene_descripcion');
		if(tiene_descripcion === 0){
			div_tiene_descripcion.hide();
			div_no_tiene_descripcion.show();
		} else {
			div_no_tiene_descripcion.hide();
			div_tiene_descripcion.show();
		}
	}
}

function tipoConversatorio() {
	var modalidad = jQuery("select[name='modalidad'] option:selected");
	var div_tipo_conversatorio = jQuery("#div-tipo-conversatorio");
	div_tipo_conversatorio.hide();
	if (modalidad.val() === '1') {
		div_tipo_conversatorio.show();
	} else {
		jQuery("input[name='tipo_conversatorio']:checked").prop("checked", false);
	}
}