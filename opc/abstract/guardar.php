...
function startsWith($haystack, $needle)
{
	...
	$ignore_fields = array("t", "palabra_clave4", "palabra_clave5", 'archivo_tl_viejo', 'tematica_tl1', 'tematica_tl2', 'tematica_tl3', 'area_tl', 'linea_transversal', 'resumen_tl', 'resumen_tl2', 'resumen_tl3','resumen_tl4','resumen_tl5', 'es_admin', 'words_total', "reglamento", "conflicto_descripcion");
...
if($result["success"]){
	...
	$cuerpo .= '
			  <tr>
                ...
                if(!empty($_SESSION["abstract"]["modalidad"])) {
                    $cuerpo .= $lang["MODALIDAD"] . ': <strong>' . $modalidad . '</strong>';
                    if (!empty($_SESSION["abstract"]["tipo_conversatorio"])){
                        $tipo_conversatorio = $core->getTipoConversatorioByID($_SESSION["abstract"]["tipo_conversatorio"]);
                        $cuerpo .= ' - <strong>'.$tipo_conversatorio.'</strong>';
                        //$cuerpo .= '<br><strong>'.$tipo_conversatorio.'</strong>';
                    }
                    $cuerpo .= '<br>';
                }
				if($_SESSION["abstract"]["area_tl"] != "")
				...
				$cuerpo .= $lang["CIUDAD"];
                if(!empty($_SESSION["abstract"]["contacto_ciudad"]))
                    $cuerpo .= ' <strong>'.$_SESSION["abstract"]["contacto_ciudad"].'</strong><br>';
                else					
					$cuerpo .= ' <strong>Falta_nombre</strong><br>';
		  ...
		  <tr>
			...
			//Agregamos titulo traducido
			if($_SESSION["abstract"]["resumen_tl5"]) {
                $cuerpo .='
                          <p><strong>'.$lang["RESUMEN5"].'</strong><br />
			...
			if($_SESSION["abstract"]["contacto_ciudad"]) {
				$cuerpo .='
				  <!--<p><strong>'.$lang["CIUDAD"].'</strong><br />-->
					'.$_SESSION["abstract"]["contacto_ciudad"].'
					</p>';
			}
			//Agregamos bibliografia despues del primer resumen resumen_tl4
			if($_SESSION["abstract"]["resumen_tl4"]) {
                $cuerpo .='
                          <p><strong>'.$lang["RESUMEN4"].'</strong><br />
   ...
   /* unset($mails_congreso_admin);*/
	...
	//echo $_SESSION["abstract"]["login"];die;
//seccion de email que envia los correos a los email alternativos
if($_SERVER["SERVER_NAME"] == "localhost"){
        $mails_congreso[] = "gegamultimedios@gmail.com";
    } else {
	if(!$_SESSION["abstract"]["is_admin"] && !$_SESSION["admin"]){	
	if(!$_SESSION["abstract"]["login"]){
	if(!empty($_SESSION["abstract"]["contacto_mail"]))
		$mails_congreso[] = $_SESSION["abstract"]["contacto_mail"];
...
if($_SESSION["usuario"]=="gaston"){	
$mailOBJ->Subject = $asunto.($_SESSION["abstract"]["is_admin"]?"[GASTON]":"");	
}else if($_SESSION["usuario"]=="andrea"){
$mailOBJ->Subject = $asunto.($_SESSION["abstract"]["is_admin"]?"[ANDREA]":"");	
}else{
$mailOBJ->Subject = $asunto.($_SESSION["abstract"]["is_admin"]?"[ADMIN]":"");	
    ...
    $mailOBJ->Body = $cuerpo;
	if($_SESSION["usuario"]=="gaston"){
	//echo $_SESSION["abstract"]["is_admin"];die;
$mailOBJ->Subject = $asunto.($_SESSION["abstract"]["is_admin"]?"[GASTON]":"");	
}else if($_SESSION["usuario"]=="andrea"){
$mailOBJ->Subject = $asunto.($_SESSION["abstract"]["is_admin"]?"[ANDREA]":"");	
}else{
