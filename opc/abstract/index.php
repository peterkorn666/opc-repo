<?php
	...
	//Reedireccion a modificar trabajo si no eres admin
	//Se le hacen comentarios cuando se va a trabajar en local
	if(!$_SESSION["abstract"]["is_admin"] && empty($_SESSION["abstract"]["id_tl"]) && !$_SESSION["admin"] && ($_GET["u"] != 1)){
		//Cierre por los organizadores de acceso a modificaciones
...
<head>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            ...
            CKEDITOR.replace( 'resumen_tl3',{
                ...
                height:200,
                enterMode : CKEDITOR.ENTER_BR,
                extraPlugins: 'simpleuploads,specialchar,htmlwriter,notification'
            });
	...
	<!--Script para mostrar alerta en funcion del valor de radio button-->
	<script type="text/javascript">
	$(document).ready(function()
		{
			$("input[name=tipo_conversatorio]").click(function () {    
				
				if($('input:radio[name=tipo_conversatorio]:checked').val()==1){
					alert("Recuerde que no puede excederse de las 1500 palabras");
				}else
					alert("Recuerde que no puede excederse las 3000 palabras");
			});
		});
		
		</script>
	<!--END Script para mostrar alerta en funcion del valor de radio button-->
...
<body>
    ...
    <div class="container">
        ...
        <div align="center">
            ...
            ?>
<!--Div que muestra el acceso a modificar trabajo existente-->
            <?php
            if($_SESSION["admin"]){
                if(empty($_SESSION["abstract"]["id_tl"])){
                    ?>
        ...
        <div align="center">
            <form action="guardar.php" name="peview" method="post" enctype="multipart/form-data">
                ...
                <div id="effect_one" class="col-md-10">
                    ...
                    <h3 style="font-weight: bold;"><?=$lang["GRACIAS_POR_RESPONDER"]?></h3><br>
                ...
                <div id="mod_step1" class="col-md-10">
                    ...
                    <div align="center">
                        ...
                        <div id="div-tipo-conversatorio" class="col-md-8 text-left" style="display: none; font-size:
                        14px; margin-top: 15px;">
                            <?php
                            $tipos_conversatorios = $trabajos->getTiposConversatorios();
                            foreach($tipos_conversatorios as $tipo_conversatorio)
                            {
                                if($tipo_conversatorio["id_tipo_conversatorio"]==$_SESSION["abstract"]["tipo_conversatorio"])
                                    $chk = "checked";
                                ?>
                                <input type="radio" name="tipo_conversatorio"
                                       value="<?=$tipo_conversatorio["id_tipo_conversatorio"]?>" <?=$chk?>>
                                <?=$tipo_conversatorio["descripcion_tipo_conversatorio_".$_SESSION["abstract"]["lang"]]?><br>
                                <?php
                                $chk = "";
                            }
                                        ...
                                        <div class="row div_persona_ya_registrada">
                                            <div class="col-md-4 sin_padding_izquierdo padding_horizontal">
                                                ...
                                                if ($_SESSION["abstract"]["inscripto_" . $i] == 1) {
													echo "<strong style='color: green;'>".$lang['PERSONA_PAGA']."</strong>";
                                                    ?>
                                                    <div class="div_constancia">
                                                        <a href="acp/carta_aceptacion.php?t=<?= base64_encode($_SESSION["abstract"]["id_tl"]) ?>&k=<?= base64_encode($_SESSION["abstract"]["id_autor_" . $i]) ?>"
                                                           style="color: #007bff;"><?=$lang['CARTA_ACEPTACION']?></a><br/>
                                                        <a href="acp/ponente.php?t=<?= base64_encode($_SESSION["abstract"]["id_tl"]) ?>&k=<?= base64_encode($_SESSION["abstract"]["id_autor_" . $i]) ?>"
                                                           style="color: #007bff;"><?=$lang['CONSTANCIA_ASISTENCIA']?></a><br/>

														<a href="acp/certificado_autor.php?t=<?= base64_encode($_SESSION["abstract"]["id_tl"]) ?>&k=<?= base64_encode($_SESSION["abstract"]["id_autor_" . $i]) ?>"
                                                           style="color: #007bff;"><?=$lang['CERTIFICADO_AUTOR']?></a>
                                                    </div>
                                                <?php
                                                } else {
												...
												?>													
													   <br/><a href="acp/carta_aceptacion.php?t=<?= base64_encode($_SESSION["abstract"]["id_tl"]) ?>&k=<?= base64_encode($_SESSION["abstract"]["id_autor_" . $i]) ?>"
                                                           style="color: #007bff;"><?=$lang['CARTA_ACEPTACION']?></a><br/>
												<?php                                                    
                                                }
                                                ?>
                    ...
                    <hr />
                        ...
                        <div id="div_resumen_tl">
                           ...
                           <!--END Contador de palabras visible 1-->
                            ...
                            <textarea id="resumen_tl5" name="resumen_tl5" class="tiny"><?=$_SESSION["abstract"]["resumen_tl5"]?></textarea>
						   <!--END titulo traducio-->
							...
							<div class="titulo_box text-left ">
								...
								<div class="col-md-5 sin_padding_izquierdo">
									<input type="text" name="contacto_ciudad" value="<?=$_SESSION["abstract"]["contacto_ciudad"]?>" class="form-control" />
								</div>
                            </div>
							<!--END Nombre del traductor-->
							
							<!--Bibliografia del resumen original-->
						   <!--Newww-->
						   <br>
                            <div class="titulo_box text-left">
                                <?=$lang['RESUMEN4']?>
                            </div>
                            <textarea id="resumen_tl4" name="resumen_tl4" class="tiny"><?=$_SESSION["abstract"]["resumen_tl4"]?></textarea>
						    <!--END Bibliografia del resumen origunal-->
							
							<!--Contador de palabras visible 2
							<input type="hidden" name="words_totals1" value="0"  />
                            <div id="limits_words1" style="margin:0 0 30px 0;" class="text-left">
                                <?=$lang['TOTAL_PALABRAS']?>: <span id="totalwords1"></span>
                            </div>
                           <!--END Contador de palabras visible 2-->
                        ...
                        <div align="left">
                            <div class="col-md-8">
                                <div class="row">
									...
									<!--Etiqueta para el certifico
									<div class="col-md-6">
										<a href="acp/certificado_autor.php?t=<?= base64_encode($_SESSION["abstract"]["id_tl"]) ?>&k=<?= base64_encode($_SESSION["abstract"]["id_autor_" . $i]) ?>" style="color: green;">Etiqueta</a>
									</div>-->
                                </div>
                            </div>
                        </div><br>
	...
	<!--Div que muestra el link para subir el archivo completo del trabajo-->

	<!--END Div que muestra el link para subir el archivo completo del trabajo-->
