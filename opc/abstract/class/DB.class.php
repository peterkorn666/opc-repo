    ...
    public function query($sql, $params = array()) {
        ...
        if($this->_query = $this->_pdo->prepare($sql)) {
            ...
            if($this->_query->execute()) {
                ...
                $allow = true;
                foreach ($deny as $d){
                    if(strpos(strtolower($sql), $d)!==false)
                        $allow = false;
                }
                if($allow)
                    $this->_results = $this->_query->fetchAll(PDO::FETCH_ASSOC);
                $this->_count = $this->_query->rowCount();
            } else {$this->_error = true; $this->_errorInfo = $this->_pdo->errorInfo(); echo '<div class="alert alert-danger text-center"><h3><b>¡ATTENTION!</b><br>An error has occurred, contact the administrator.<br>Your data has been lost.</h3></div>';}
        }
        return $this;
    }
