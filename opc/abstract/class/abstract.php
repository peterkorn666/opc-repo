...
class abstracts{
    ...
    public function getTiposConversatorios(){
        return $this->db->query("SELECT * FROM trabajos_libres_conversatorios_tipos ORDER BY orden")->results();
    }
	...
	private function saveTL(){	
		...
		//Nuevo titulo traducido
		if($_SESSION["abstract"]["resumen_tl5"] == ""){
            $resumen_tl5 = NULL;
        }else{
            $resumen_tl5 = $_SESSION["abstract"]["resumen_tl5"];
        }
        ...
        if($_SESSION["abstract"]["tipo_conversatorio"] != ""){
            $tipo_conversatorio = (int)$_SESSION["abstract"]["tipo_conversatorio"];
        }else
            $tipo_conversatorio = NULL;
		
		$data = array(
		    //"reglamento"=>(int)$_SESSION["abstract"]["reglamento"],
            //"conflicto_descripcion"=>$_SESSION["abstract"]["conflicto_descripcion"],
			"modalidad" => $modalidad,
			"tipo_conversatorio" => $tipo_conversatorio,
