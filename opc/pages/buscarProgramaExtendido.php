...
$unicosArraysDistintos = array();
		...
		if(count($arrayBusqueda)>0){
			...
			foreach($arrayBusqueda as $key => $busqueda){
                ...
                $sqlSearch .= "EXISTS(SELECT 1 FROM paises c WHERE p.Pais = c.ID_Paises AND c.Pais LIKE '%$busqueda%') OR ";
                $sqlSearch .= "EXISTS(SELECT 1 FROM instituciones i WHERE p.Institucion = i.ID_Instituciones AND i.Institucion LIKE '%$busqueda%') OR ";
                $sqlSearch .= "EXISTS(SELECT 1 FROM instituciones i_otro WHERE p.Institucion_otro = i_otro.ID_Instituciones AND i_otro.Institucion LIKE '%$busqueda%') OR ";
		...
		if($_GET["key"]){
		 ...
		 if(count($unicosArraysDistintos)>0){//
		 	...
		 	$sql6 = "SELECT * FROM cronograma as c JOIN salas as s ON c.section_id=s.salaid WHERE ";
		 	$bucle6 = 0;
			$sql6 .= "(";
			...
			$sql6 .= ")";
		 	$sql6 .= " ORDER BY SUBSTRING(start_date,1,10),SUBSTRING(start_date,11,5)  ASC;";
            //echo $sql6;
		 	$rs = $core->query($sql6);
            ...
            $helper = 0;
		 	foreach($rs as $row){
				...
				$templates->programaExtendido($row,$dia_,$sala_,$helper, true);
