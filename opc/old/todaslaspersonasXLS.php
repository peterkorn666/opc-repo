<?
header("Content-Disposition: attachment; filename=Todos_las_personas_".date("Y-m-d").".xls");
...
$rsTOTAL = mysqli_query($sqlTOTAL, $con) or die(mysqli_error());
?>
<meta charset="utf-8">
...
<tr>
	...
	<?
while($totalTLS = mysqli_fetch_array($rsTOTAL)){
...
<tr>
        ...
        <td><?php
			...
			{
        		$gSala = mysqli_query("SELECT * FROM salas WHERE salaid='{$totalTLS["section_id"]}'", $con);
				$sala = mysqli_fetch_array($gSala);
				echo $sala["name"];
			}
			?>
        </td>
		...
		<td><?php
				...
				{
					$gIns = mysqli_query("SELECT * FROM instituciones WHERE ID_Instituciones='{$totalTLS["institucion"]}'", $con);
					$ins = mysqli_fetch_array($gIns);
		...
		<td><?php
			if($totalTLS["pais"])
			{
				$gPais = mysqli_query("SELECT * FROM paises WHERE ID_Paises='{$totalTLS["pais"]}'", $con);
				$pais = mysqli_fetch_array($gPais);
				echo $pais["Pais"];
			}
			?></td>
...
<?        
		...
		while($row = mysqli_fetch_array($rs)){
				if($row["en_calidad"]!=""){
					$gRoles = mysqli_query("SELECT * FROM calidades_conferencistas WHERE ID_calidad='{$row["en_calidad"]}'", $con);
					$rol = mysqli_fetch_array($gRoles);
					$roles .= $rol["calidad"].", ";
				}
		}	
...
<tr>
    ...
    <td><?php
        if($totalTLS["pais"])
        {
            $gPais = mysqli_query("SELECT * FROM paises WHERE ID_Paises='{$totalTLS["pais"]}'");
            $pais = mysqli_fetch_array($gPais);
            echo $pais["Pais"];
        }
        ?></td>
