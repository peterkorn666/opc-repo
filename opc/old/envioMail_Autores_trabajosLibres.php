...
$rs = $con->query($sql);
while($row = $rs->fetch_array()){
	$sql2 = "SELECT tp.ID, t.estado FROM trabajos_libres_participantes tp JOIN trabajos_libres t ON tp.ID_trabajos_libres=t.id_trabajo WHERE tp.ID_participante = " . $row["ID_Personas"] . ";";
	$rs2 = $con->query($sql2);
	while($row2 = $rs2->fetch_array()){
		//if(($apellidoAnt != $row["Apellidos"])&&($mailAnt != $row["Mail"])){
			array_push($arrayPersonas, array($row["Apellidos"], $row["Nombre"], $row["Mail"], $row["Pais"], $row["ID_Personas"], $row2["ID"], "autor_inscripto" => $row["inscripto"], "estado_trabajo" => $row2["estado"]));
			$apellidoAnt = $row["Apellidos"];
			$mailAnt = $row["Mail"];
		//}
	}
            ...
            <tr>
              <td height="25" colspan="2" valign="middle" bgcolor="#D0E1D1"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td>
                    	<input id="filtro_no_inscriptos" name="filtro_no_inscriptos" type="checkbox" value="1">Solo NO inscriptos
                    </td>
                    <td>
                    	<input id="filtro_trabajos_aprobados" name="filtro_trabajos_aprobados" type="checkbox" value="1">Solo trabajos aprobados
                    </td>
                    <td>
                    	<input id="filtro_no_repetidos" name="filtro_no_repetidos" type="checkbox" value="1">Sin autores repetidos
                    </td>
                    </tr>
              </table></td>
            </tr>
...
foreach ($arrayPersonas as $i){
	?>
            ...
            <tr style="font-size:11px" >
              <td width="21" bgcolor="#FFFFFF" ><input name="autor[]" type="checkbox" id="autor[]" data-inscripto="<?=$i["autor_inscripto"]?>" data-estado_trabajo="<?=$i["estado_trabajo"]?>" data-nombre="<?=$i[1]?>" data-apellido="<?=$i[0]?>" data-email="<?=$i[2]?>" value="<?=$i[4]?>" /></td>
