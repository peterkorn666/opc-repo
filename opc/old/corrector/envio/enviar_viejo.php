...
require ("../../web/clases/class.Traductor.php");
require "../../web/clases/class.Cartas.php";
...
//$mailOBJ->From    		= "familiascoloquio@gmail.com";
$mailOBJ->FromName 		= "Pediatria 2017";
$mailOBJ->IsHTML(true);
$mailOBJ->Timeout=120;
...
$sql = "SELECT * FROM inscriptos WHERE $filtro AND email<>'' ORDER BY ID_Personas";
...
if($nombre_conferencista==1){
	$asunto .= " - [".$row->ID_Personas."] ".$row->nombre." ".$row->apellido;
}
...
if($_POST["predefinido"]!=""){
	...
	if ($predefinida = mysql_fetch_array($rs)){
		...
		$cartaMail = $cartas->personificarPorInscripto($cartaMail, $row->ID_Personas);
		$base64 = base64_encode($row->ID_Personas);
		$cartaMail = str_replace("<:participaciones>", $imprimir , $cartaMail);
		$link_conf = "<a href='http://coloquiofamilias2015.org/programa/consulta/actividad.php?idP=$base64'>http://coloquiofamilias2015.org/programa/consulta/actividad.php?idP=$base64</a>";
		$cartaMail = str_replace("<:link_conferencista>", $link_conf , $cartaMail);
	}
}else{
	$base64 = base64_encode($row->ID_Personas);
	$link_conf = "<a href='http://coloquiofamilias2015.org/programa/consulta/actividad.php?idP=$base64'>http://coloquiofamilias2015.org/programa/consulta/actividad.php?idP=$base64</a>";
	$mensaje = str_replace("<:link_conferencista>", $link_conf , $_POST["mensaje"]);
...
<table width="626" border="0" cellspacing="1" cellpadding="4" align="center">
  <tr>
    <td colspan="2" align="center" style="font-size:16px"><img src="http://coloquiofamilias2015.org/programa/images/banner.jpg"></td>
  </tr>
  <tr>
...
if($mails_backOk!=""){
	echo trim(utf8_decode($row->nombre)." ".utf8_decode($row->apellido)." - ".$mails_backOk,"<br>");
}
if($mails_backError!=""){
	echo trim(utf8_decode($row->nombre)." ".utf8_decode($row->apellido)." - ".$mails_backError,"<br>");
}
?>
