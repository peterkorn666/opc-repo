...
function BotonElimiar(cual){
	if(cual>0){
		document.getElementById("divEliminarCoAutor").innerHTML= "<a href=\"#fin\" class=\"textos\" onClick=\"eliminarDivCoAutor("+cual+")\">[-] Eliminate the last Co-author</a>";
	}else{
...
function validar(){
	...
	if(areaTLSeleccionada==false){
		alert("Please, Categories of work selects one");
		return;
	}
	...
	if(archivoAdjuntado == 0){
		alert("Please, it must enclose some file of the work.")
...
function adjuntarArchivo(){
	...
	if(document.form1.archivo.value==""){
		alert("Please, it selects to a file *.doc or *.rtf to be enclosed");
		document.form1.archivo.focus();
		return; 
	}
	...
	if(extension=="doc" || extension=="rtf" ){
		...
		alert("The selected file must be * doc or * rtf");
		document.form1.archivo.focus();
		return; 
	}
