...
include("../conexion.php");
	//SUCS
	$sql = "SELECT * FROM congreso ORDER by Dia_orden,Sala_orden,Hora_inicio;";
...
if( ($row["Tipo_de_actividad"]=="Plenaria") &&  ($row["Sala"]=="Sala Florida - SUCS")){}else{
...
/**/
	...
	*/	
	...
	if($row["Trabajo_libre"]==1){
		...
		$sql_t ="SELECT * FROM trabajos_libres WHERE ID_casillero= '" . $row["Casillero"] . "' AND estado<>3 AND tipo_tl='Oral' ORDER BY numero_tl ASC;";
		$rs_t = mysql_query($sql_t,$con);
		while ($row_t = mysql_fetch_array($rs_t)){
