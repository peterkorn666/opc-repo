...
//	function selectTL_estado($cual, $ubi, $are, $tip, $cla, $ins){
	...
	function Autores($id_trabajo){
		...
		while($row = $rs->fetch_array()){
			...
			$imprimir .= "<span style='font-family:Candara, Times New Roman, Times, serif; font-size: 16px;color: #000000;margin:0px'>";
			...
			for ($i=0; $i < count($arrayPersonas); $i++){
				...
				$imprimir .= $arrayPersonas[$i][2]." ".$arrayPersonas[$i][1];
				if($primero){
					//$PrimerAutor = $arrayPersonas[$i][1] . ", " . $arrayPersonas[$i][2];
					$PrimerAutor = $arrayPersonas[$i][2]." ".$arrayPersonas[$i][1];
		...
		$imprimir .= "<span style='font-family: Candara, Times New Roman, Times, serif;font-size: 14px;color: #000000;margin:0px'>";
