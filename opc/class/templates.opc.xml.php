...
class templateOPC extends core
{
		...
		for ($i=0; $i < count($arrayPersonas); $i++){
			...
			$gestionAutores .= "<div class='conferencista'>";
				...
				if(!$presentador){
					...
					/*$gestionAutores .= " <span class='autor_pais' style='font-style: italic;'>(".$this->getPais($arrayPersonas[$i][5])["Pais"].")</span>";*/
					if($mail !== false){
	...
	public function templateAutoresTL($data, $presentador=false, $crono="")
	{
		...
		foreach($sql as $key => $row){	
			...
			$inscripto[$key] = $row["inscripto"];
            if($institucion[$key] == 'Otra'){
                array_push($arrayPersonas, array($row["Institucion_otro"],$nombre[$key], $apellido[$key],  $lee[$key],
                    $inscripto[$key],$pais[$key]));
                array_push($arrayInstituciones , $row["Institucion_otro"]);
            } else{
                array_push($arrayPersonas, array($institucion[$key],  $nombre[$key], $apellido[$key],  $lee[$key], $inscripto[$key],$pais[$key]));
                array_push($arrayInstituciones , $institucion[$key]);
            }
		} 
		...
		for ($i=0; $i < count($arrayPersonas); $i++){
			if($i>0){
				...
				$primerAutor .= $arrayPersonas[$i][1]." <b>".$arrayPersonas[$i][2] . "</b>";
			if($arrayPersonas[$i][0]!=""){
				$claveIns = (array_search($arrayPersonas[$i][0] ,$arrayInstitucionesUnicas))+1;
			}else{
				$claveIns = "";
			...
			//Esta U es para subrayar al presentador
			//if($arrayPersonas[$i][3]==1){
				//$gestionAutores .= "<u>";
			//}
			//aqui se puede agragar el pais al lado de los nombres de los autores
			if(!$presentador){
				$gestionAutores .=  $arrayPersonas[$i][1] . " " . $arrayPersonas[$i][2];
			}
			if($arrayPersonas[$i][3]==1){
				...
				//$gestionAutores .= "</u>";
				if($presentador)
					$gestionAutores .= $arrayPersonas[$i][1]. "<b>".$arrayPersonas[$i][2] ." </b>; ";
				$helper++;
				//$gestionAutores .= $primerAutor;
			}
			if($helper==1 && $i==(count($sql)-1) && $presentador)
