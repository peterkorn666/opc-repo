...
class Core{
	...
	public function getCronoByDaySala($day,$sala="",$tipo_actividad=""){
		if($day)
		{
			...
			$w = "WHERE SUBSTRING(start_date,1,10)=:start";
		}
		if($sala)
		{
	...
	public function getCronoByDayHoraSala($day="", $sala="", $hora="", $tipo_actividad=""){
		$w = "WHERE 1=1";
		if($day)
		{
			$this->bd->bind("start",$day);
		...
		if($hora)
		{
			$this->bd->bind("hora_inicio",$hora);
			$w .= " AND SUBSTRING(start_date,12,5)=:hora_inicio";
		}
		if($tipo_actividad)
		{
			$this->bd->bind("tipo_actividad",$tipo_actividad);
		...
		$sql = $this->bd->query("SELECT * FROM cronograma as c JOIN salas as s ON c.section_id=s.salaid $w ORDER BY SUBSTRING(c.start_date,1,10),c.start_date,s.orden");
		return $sql;
	}
	...
	{
		$sql = $this->bd->query("SELECT * FROM conferencistas");
		return $sql;
	}
	...
	{
		$sql = $this->bd->query("SELECT * FROM areas_trabjos_libres ORDER BY id");
		return $sql;
	}
	...
	public function getTipoTLByID($id)
	{
		$this->bd->bind("id", $id);
		$sql = $this->bd->row("SELECT * FROM tipo_de_trabajos_libres WHERE id=:id");
		return $sql;
	}
	
	public function getTematicaTLByID($id)
	{
		$this->bd->bind("id", $id);
		$sql = $this->bd->row("SELECT * FROM trabajos_libres_tematicas WHERE id=:id");
		return $sql;
	}

    public function getLineaTransversalByID($id)
    {
        $this->bd->bind("id", $id);
        $sql = $this->bd->row("SELECT * FROM trabajos_libres_lineas_transversales WHERE id=:id");
        return $sql;
    }

    public function getModalidadByID($id)
    {
        $this->bd->bind("id", $id);
        $sql = $this->bd->row("SELECT * FROM trabajos_libres_modalidades WHERE id=:id");
        return $sql;
    }

    public function getTipoConversatorioByID($id_tipo_conversatorio){
        $this->bd->bind("id", $id_tipo_conversatorio);
        $sql = $this->bd->row("SELECT * FROM trabajos_libres_conversatorios_tipos WHERE id_tipo_conversatorio=:id");
        return $sql;
    }
	...
	{
		$this->bd->bind("id", $id);
		$sql = $this->bd->row("SELECT * FROM areas_trabjos_libres WHERE id=:id");
		return $sql["Area_es"];
	}
	...
	public function insertInstitution($name)
	{
		...
		$sql = $this->bd->query("SELECT * FROM instituciones WHERE Institucion=:name LIMIT 0,1");
	...
	public function getPaisesTL()
	{
		$ids_paises = $this->query("SELECT Pais as id_pais FROM personas_trabajos_libres GROUP BY Pais");
		if(count($ids_paises) > 0){
			$txt_paises = "(";
			foreach($ids_paises as $id_pais){
				$txt_paises .= $id_pais["id_pais"].",";
			}
			$txt_paises = substr($txt_paises, 0, -1);
			$txt_paises .= ")";
		}
		$arrayPaises = $this->query("SELECT ID_Paises as id_pais, Pais as pais FROM paises WHERE ID_Paises IN ".$txt_paises." ORDER BY Pais");
		return $arrayPaises;
	}
	...
	public function getInscriptos(){
		return $this->query("SELECT * FROM inscriptos ORDER BY id");
	}
  ...
  <strong>Estado:</strong> Todos los datos se han eliminado correctamente.
	...
	public function busquedaTL($param){
		...
		if($param["filtro_pais"]!=""){
			$filtroPais = "JOIN trabajos_libres_participantes tp ON tp.ID_trabajos_libres=t.id_trabajo JOIN personas_trabajos_libres p ON tp.ID_participante=p.ID_Personas JOIN paises pa ON p.Pais=pa.ID_Paises";
		}else
			$filtroPais = "";
		...
		//if(trim($param["filtro_palabra_clave"]) != ""){
		...
		if(trim($param["filtro_palabra_clave"]) || trim($param["filtro_email_autor"])){
			...
			foreach ($arrayBusquedaClave as $i){
				...
				$sql5 .= " or resumen2 like '%$i%' ";
				$sql5 .= " or resumen3 like '%$i%' ";
				$sql5 .= " or resumen4 like '%$i%' ";
		...
		if($param["filtro_eje_tematico"]!="")
			$filtro .= " and area_tl='".$param["filtro_eje_tematico"]."'";

		if($param["filtro_tipo_tl"]!="")
			$filtro .= " and tipo_tl='".$param["filtro_tipo_tl"]."'";
		
		if($param["filtro_modalidad"]!="")
			$filtro .= " and modalidad='".$param["filtro_modalidad"]."'";

        if($param["filtro_linea_transversal"]!="")
            $filtro .= " and linea_transversal='".$param["filtro_linea_transversal"]."'";

		if($param["filtro_ubicado"]!="")
			$filtro .= " and id_crono='".$param["filtro_ubicado"]."'";		
		
		if($param["filtro_idioma"]!="")
			$filtro .= " and lang='".$param["filtro_idioma"]."'";
		
		if($param["filtro_salas"]!=""){
			$variblessala="c.start_date, c.end_date, c.id_crono as idcrono, c.section_id, c.tipo_actividad";
			$filtrosala = "JOIN cronograma as c ON t.id_crono=c.id_crono JOIN salas as s ON c.section_id=s.salaid";
			$filtrosala .= " and s.salaid='".$param["filtro_salas"]."'";
		}else{
		...
		if($param["filtro_pais"]!=""){
			$filtro .= " and pa.ID_Paises='".$param["filtro_pais"]."'";
		}
		if($param["filtro_resumo"]!=""){
			if($param["filtro_resumo"] == "Si"){
				$filtro .= " AND resumen2 IS NOT NULL";
			}else if($param["filtro_resumo"] == "No"){
				$filtro .= " AND resumen2 IS NULL";
			}
		}
		if($param["filtro_bibliografia"]!=""){
			if($param["filtro_bibliografia"] == "Si"){
				$filtro .= " AND resumen3 IS NOT NULL";
			}else if($param["filtro_bibliografia"] == "No"){
				$filtro .= " AND resumen3 IS NULL";
			}
		}
		
		/*if($param["filtro_bibliografia"]!=""){
			if($param["filtro_bibliografia"] == "Si"){
				$filtro .= " AND resumen4 IS NOT NULL";
			}else if($param["filtro_bibliografia"] == "No"){
				$filtro .= " AND resumen4 IS NULL";
			}
		}*/
		...
		t.titulo_tl, t.resumen, t.resumen2, t.resumen3, t.resumen4, t.Hora_inicio, t.Hora_fin, t.archivo_tl, t.contacto_mail, t.idioma_tl, '$variblessala'
		
		FROM trabajos_libres t $filtrosala $filtroPais $filtro  GROUP BY t.id_trabajo order by t.numero_tl ASC ". $inicio;
		...
		if($param["filtro_presentador"]=="todos" || $param["filtro_presentador"]==""){
			...
			foreach($result as $row123){
					if($param["filtro_presentador"]==2){
						$sql12 = "SELECT * FROM trabajos_libres_participantes  WHERE ID_trabajos_libres  = '" . $row123["id_trabajo"]."'";
				$result2 = $this->query($sql12);
					
					foreach($result2 as $row12){						
					
					$sql1 = "SELECT * FROM personas_trabajos_libres  WHERE ID_Personas = '" . $row12["ID_participante"]."'  AND inscripto='1'";
	...
	public function showUbicacion($start_date, $end_date, $section){
		...
		return ucwords(utf8_encode(strftime($config["time_format"],strtotime($this->helperDate($start_date,0,10))))).' | '.$this->getSalaID($section)['name'].' | '.$this->helperDate($start_date,10,6).' - '.$this->helperDate($end_date,10,6);
	}
	...
	public function getCronoConferencistasByConf($id_conf)
	{
		$this->bd->bind("id",$id_conf);
		$sql = $this->bd->query("SELECT * FROM crono_conferencistas WHERE id_conf = :id ORDER BY id");
		return $sql;
	}
	
	public function getConferencistasTipos(){
		return $this->bd->query("SELECT * FROM conferencistas_tipos ORDER BY orden");
	}
}
?>