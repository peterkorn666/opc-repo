...
$presentacion->abrir("destinatariosDesdeBase.php","../paginas/","Principal");

require "../clases/form_br.php";

$form_br_class1= new form_br_class();
$form_br_class1->action="destinatariosDesdeBase.php";
...
function agregarSusbscriptosSeleccionados() {
	...
	$sqlBase2="update subscriptos set subscriptos.EnviosPendientes=CONCAT(subscriptos.EnviosPendientes,$idEnvio,\":\")";
	$sqlBase2.=" where subscriptos.Activo=1 and not (subscriptos.EnviosPendientes like '%:$idEnvio:%') and subscriptos.IdSubscripto in ";
	$largoMaximoSQL=1024;
	$in="";
	$i=0;
	while ($i<count($subscriptos)) {
		if ($in<>"" && strlen($sqlBase . " (" . $in . "," . $subscriptos[$i] . ")")>$largoMaximoSQL) {
			...
			$agregados = $agregados + $sistema->database->filasAfectadas;
	...
	$form_br_class1->mensaje=$agregados . " subscriptos agregados al envío";
...
function agregarSubscriptosTodos() {
	...
	$whereGrupos="1=1";
	if ($idGrupo>0) {
		$sql="insert into envios_subscriptos (IdEnvio,IdEstadoEnvio,IdSubscripto,Email)";
		$sql.=" select $idEnvio,1,subscriptos.IdSubscripto,subscriptos.Email from subscriptos";
		$sql.=" LEFT JOIN subscriptos_grupos ON subscriptos_grupos.IdSubscripto=subscriptos.IdSubscripto ";
		$sql.=" LEFT JOIN subscriptos_datos_personales ON subscriptos.IdSubscripto=subscriptos_datos_personales.IdSubscripto";
		$sql.=" where subscriptos.Activo=1 and not (subscriptos.EnviosPendientes like '%:$idEnvio:%')";
		$sql.=" and (" . $form_br_class1->condicionBuscar() . ")";
		$sql.=" and subscriptos_grupos.IdGrupo=$idGrupo";
		if ($form_br_class1->limite!="") {
			$sql.=" LIMIT " . $form_br_class1->limite;
		}
		$sistema->database->ejecutarSentenciaSQL($sql);	
								
		$sql="update subscriptos,subscriptos_datos_personales,subscriptos_grupos set EnviosPendientes=CONCAT(EnviosPendientes,$idEnvio,\":\")";
		$sql.=" where subscriptos_datos_personales.IdSubscripto=subscriptos.IdSubscripto and subscriptos_grupos.IdSubscripto=subscriptos.IdSubscripto and subscriptos_grupos.IdGrupo=$idGrupo and Activo=1 and not (subscriptos.EnviosPendientes like '%:$idEnvio:%') ";
		$sql.=" and (" . $form_br_class1->condicionBuscar() . ")";
		if ($form_br_class1->limite!="") {
			$sql.=" LIMIT " . $form_br_class1->limite;
		}
		...
		$sql="update subscriptos,subscriptos_datos_personales set EnviosPendientes=CONCAT(EnviosPendientes,$idEnvio,\":\")";
		$sql.=" where subscriptos_datos_personales.IdSubscripto=subscriptos.IdSubscripto and  subscriptos.Activo=1 and not (subscriptos.EnviosPendientes like '%:$idEnvio:%') ";
		$sql.=" and (" . $form_br_class1->condicionBuscar() . ")";
		if ($form_br_class1->limite!="") {
			$sql.=" LIMIT " . $form_br_class1->limite;
		}
