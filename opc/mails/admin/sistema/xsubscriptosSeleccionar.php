...
$presentacion->abrir("subscriptosSeleccionar.php","../paginas/","Principal");

require "../clases/form_br.php";

$form_br_class1= new form_br_class();
$form_br_class1->action="subscriptosSeleccionar.php";
...
$presentacion->asignarValor("envios","<option value=-1>Seleccione mail</option>".cargarListBoxRS($envios->buscarDatos("IdEnvio,CONCAT('(',IdEnvio,') ',Descripcion)","","IdEnvio"),$idEnvio));
...
function agregarSubscriptosTodos() {
	...
	if ($idGrupo>0) {
		...
		$sql.=" select $idEnvio,1,subscriptos.IdSubscripto,Email from subscriptos";
		$sql.=" LEFT JOIN subscriptos_grupos ON subscriptos_grupos.IdSubscripto=subscriptos.IdSubscripto ";
		$sql.=" where Activo=1 and not (subscriptos.EnviosPendientes like '%:$idEnvio:%')";
		...
		$sql="update subscriptos,subscriptos_grupos set EnviosPendientes=CONCAT(EnviosPendientes,$idEnvio,\":\")";
		$sql.=" where subscriptos_grupos.IdSubscripto=subscriptos.IdSubscripto and subscriptos_grupos.IdGrupo=$idGrupo and Activo=1 and not (subscriptos.EnviosPendientes like '%:$idEnvio:%') ";
		$sql.=" and (" . $form_br_class1->condicionBuscar() . ")";
		if ($form_br_class1->limite!="") {
			$sql.=" LIMIT " . $form_br_class1->limite;
		}
		...
		$sql="insert into envios_subscriptos (IdEnvio,IdEstadoEnvio,IdSubscripto,Email)";
		$sql.=" select $idEnvio,1,IdSubscripto,Email from subscriptos";
		$sql.=$sqlJoinGrupos;
		$sql.=" where Activo=1 and not (subscriptos.EnviosPendientes like '%:$idEnvio:%')";
		$sql.=" and (" . $form_br_class1->condicionBuscar() . ")";
		$sql.=" and (" . $whereGrupos . ")";
		...
		$sql="update subscriptos set EnviosPendientes=CONCAT(EnviosPendientes,$idEnvio,\":\")";
		$sql.=" where Activo=1 and not (subscriptos.EnviosPendientes like '%:$idEnvio:%') ";
		$sql.=" and (" . $form_br_class1->condicionBuscar() . ")";
		if ($form_br_class1->limite!="") {
			$sql.=" LIMIT " . $form_br_class1->limite;
	...
	$form_br_class1->mensaje=$sistema->database->filasAfectadas . " subscriptos agregados al envío";
...
CONCAT(LEFT(EnviosPendientes,LOCATE(':$idEnvio:',EnviosPendientes)), RIGHT(EnviosPendientes, LENGTH(EnviosPendientes)-LOCATE(':$idEnvio:',EnviosPendientes)-LENGTH(':$idEnvio:')+1)),EnviosPendientes) where IdSubscripto in ";
	...
	$sql="select IdSubscripto from subscriptos ";
	$sql.=" where (" . $form_br_class1->condicionBuscar() . ")";
	if ($form_br_class1->limite!="") {
		$sql.=" LIMIT " . $form_br_class1->limite;
	}
	
	$sistema->database->ejecutarSentenciaSQL($sql);
	$rsSubscriptos=new RS($sistema->database->rs);

	$largoMaximoSQL=1024;
	$in="";
	$i=0;
	while ($row=$rsSubscriptos->darSiguienteFila()) {
		if ($in<>"" && strlen($sqlBase . " (" . $in . "," . $row["IdSubscripto"] . ")")>$largoMaximoSQL) {
			$sql=$sqlBase . " (" . $in .")";
			$sistema->database->ejecutarSentenciaSQL($sql);			
			$sql=$sqlBase2 . " (" . $in .")";			
			$sistema->database->ejecutarSentenciaSQL($sql);	
