...
$rule_categories = array(
   'antispam'       => array('remove'=>1,'bounce_type'=>'blocked'  )
  ,'unauthorized'   => array('remove'=>0,'bounce_type'=>'blocked')
  ,'autoreply'      => array('remove'=>1,'bounce_type'=>'autoreply')
  ...
  ,'internal_error' => array('remove'=>1,'bounce_type'=>'soft')
  ,'defer'          => array('remove'=>1,'bounce_type'=>'soft'     )
  ,'delayed'        => array('remove'=>1,'bounce_type'=>'soft')
  ,'dns_loop'       => array('remove'=>1,'bounce_type'=>'hard'     )
  ,'dns_unknown'    => array('remove'=>1,'bounce_type'=>'hard'     )
  ,'full'           => array('remove'=>1,'bounce_type'=>'soft'     )
  ,'inactive'       => array('remove'=>1,'bounce_type'=>'hard'     )
  ,'latin_only'     => array('remove'=>1,'bounce_type'=>'soft'     )
  ,'other'          => array('remove'=>1,'bounce_type'=>'generic'  )
  ,'oversize'       => array('remove'=>1,'bounce_type'=>'soft'     )
  ,'outofoffice'    => array('remove'=>1,'bounce_type'=>'soft'     )
  ,'unknown'        => array('remove'=>1,'bounce_type'=>'hard'     )
  ,'server_unknown' => array('remove'=>1,'bounce_type'=>'hard'     )
  ,'unrecognized'   => array('remove'=>0,'bounce_type'=>false,     )
  ,'user_reject'    => array('remove'=>1,'bounce_type'=>'hard'     )
  ,'warning'        => array('remove'=>1,'bounce_type'=>'soft'     )
);
...
function bmhBodyRules($body,$structure,$debug_mode=false) {
   ...
   * <xxxxx@yourdomain.com>:
   * 111.111.111.111 does not like recipient.
   * Remote host said: 550 5.1.1 <xxxxx@yourdomain.com>: Recipient address rejected: yourdomain.com
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Recipient address rejected/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0236';
    $result['email']       = $match[1];
  }
  ...
  /*
   * xxxxx@yourdomain.com cannot be delivered. This account has been disabled or discontinued 
   */
  elseif (preg_match ("/(\S+@\S+\w) cannot be delivered. This account has been disabled or discontinued/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0236';
    $result['email']       = $match[1];
  }
  ...
  /*
   * xxxxx@yourdomain.com: 5.1.1 User unknown
   */
  elseif (preg_match ("/(\S+@\S+\w): 5.1.1 User unknown/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0236';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)> not known at this site/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0236';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/(\S+@\S+\w);.*bad destination mailbox address/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0236';
    $result['email']       = $match[1];
  }
    ...
    elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*\n?.*no mailbox here by that name/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0236';
    $result['email']       = $match[1];
  }
	
    elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*este destinatario nao existe/",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0236';
    $result['email']       = $match[1];
  }
  ...
  /*
   * <xxxxx@yourdomain.com>: Recipient address rejected: User unknown in virtual mailbox table
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*User unknown/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  }

  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*This user doesn\'t have a yahoo.com/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)> was not found in LDAP server /i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  }  
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Account disabled/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/was not delivered to.*\n?.*\n?.*(\S+@\S+\w).*\n?.*\n?.*because.*\n?.*\n?.*not listed in public Name/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/was not delivered to.*\n?.*\n?.*(\S+@\S+\w).*\n?.*\n?.*because.*\n?.*\n?.*not unique/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/was not delivered to.*\n?.*\n?.*(\S+@\S+\w).*\n?.*\n?.*because.*\n?.*\n?.*Error transferring to.*Maximum hop count exceeded/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  }
  
   elseif (preg_match ("/(\S+@\S+\w).*\n?.*\n?.*\n?.*service level for the user\'s mailbox doesn\'t allow that operation/",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  } 
  ...
  /*
   * xxxxx@yourdomain.com unknown user account
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*unknown user/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0237';
    $result['email']       = $match[1];
  }
   ...
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*The email account that you tried to reach does not exist/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0238';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*No Such User/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*this recipient is not in my validrcptto list/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*does not like recipient/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*sorry, mail to that recipient is not accepted/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Address rejected/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*No existe ese usuario en esta direccion/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Unknown User/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*no mailbox found for user/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*sorry, no mailbox here by that name/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*This address no longer accepts mail/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*no such address/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/(\S+@\S+\w) not listed/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/(\S+@\S+\w) unknown user account/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/(\S+@\S+\w) inactiva o no existe/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }

  elseif (preg_match ("/<(\S+@\S+\w)>.*inactiva/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0239';
    $result['email']       = $match[1];
  }
   ...
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*This user doesn.*yahoo.com/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0240';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/The following recipients are unknown: (\S+@\S+\w)/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0240';
    $result['email']       = $match[1];
  }
   ...
   /*
   * <xxxxx@yourdomain.com>
   * User 'xxxxx' is not authorised to use mail.
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*is not authorised to use mail/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0240';
    $result['email']       = $match[1];
  }
   ...
   *   Invalid recipient
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*Invalid.*(?:alias|account|recipient|address|email|mailbox|user)/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0235';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*mail server permanently rejected message/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0235';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/Error en la entrega a los siguientes destinatarios\..*\n?.*.*\n?.*(\S+@\S+\w)/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0235';
    $result['email']       = $match[1];
  }  

  elseif (preg_match ("/Unable to deliver message to the following recipients.*\n?.*.*\n?.*(\S+@\S+\w)/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0235';
    $result['email']       = $match[1];
  }    
...
/*
   * rule: mailbox unknow;
   * sample:
   * The following recipients are unknown: xxxxx@yourdomain.com
   */
  elseif (preg_match ("/The following recipients are unknown: (\S+@\S+\w)/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0235';
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Bad destination email address/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0235';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*Mailbox full/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0182';
    $result['email']       = $match[1];
  }

  elseif (preg_match ("/<(\S+@\S+\w)>.*Can\'t create output/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0182';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*\n?.*Can\'t Mail box full/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0182';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*ha sobrepasado su espacio en disco/",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0182';
    $result['email']       = $match[1];
  }  
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*Mailbox full/",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0182';
    $result['email']       = $match[1];
  }  
   ...
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Mailbox quota exceeded/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0167';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Insufficient system storage/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0167';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*exceeded storage allocation/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0167';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*Mailbox quota usage exceeded/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0168';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*Unable to open mailbox/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0168';
    $result['email']       = $match[1];
  }
   ...
   * rule: mailbox full;
   * sample:
   * Remote host said: 554 <xxx@yourdomain.com>: Recipient address rejected: User is over quota.
   */
  elseif (preg_match ("/<(\S+@\S+\w)>: Recipient address rejected: User is over quota./i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0169';
    $result['email']       = $match[1];
  }
   ...
   * rule: mailbox full;
   * sample:
   * <xxxxx@yourdomain.com>... La casilla esta llena / Mailbox full
   */
  elseif (preg_match ("/<(\S+@\S+\w)>\s.*bounce.*Quota exceed/i",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0168';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*error writing.*\n?.*message: File too large/",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0168';
    $result['email']       = $match[1];
  }

  elseif (preg_match ("/<(\S+@\S+\w)>.*would exceed mailbox quota/",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0168';
    $result['email']       = $match[1];
  }  
  
  elseif (preg_match ("/\'(\S+@\S+\w)\'.*\n?.*\n?.*quota exceeded/",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0168';
    $result['email']       = $match[1];
  }    
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*\n?.*Mail box full/",$body,$match)) {
    $result['rule_cat']    = 'full';
    $result['rule_no']     = '0168';
    $result['email']       = $match[1];
  }    
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Message rejected as spam by Content Filtering/i",$body,$match)) {
    $result['rule_cat']    = 'antispam';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Error: Content Rejected/i",$body,$match)) {
    $result['rule_cat']    = 'antispam';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/(\S+@\S+\w).*\n?.*\n?.*Content of message was rejected by filter/i",$body,$match)) {
    $result['rule_cat']    = 'antispam';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/(\S+@\S+\w).*\n?.*\n?.*was considered unsolicited bulk/i",$body,$match)) {
    $result['rule_cat']    = 'antispam';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Possible email abuse detected/",$body,$match)) {
    $result['rule_cat']    = 'antispam';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*matches a profile the Internet community may consider spam/",$body,$match)) {
    $result['rule_cat']    = 'antispam';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
}

  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*demasiados usuarios de correo desconocidos/",$body,$match)) {
    $result['rule_cat']    = 'antispam';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
}

  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*Message rejected due to content restrictions/",$body,$match)) {
    $result['rule_cat']    = 'antispam';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
}
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*\n?.*que has usado para enviar tu mensaje.*\n?.*este grupo/i",$body,$match)) {
    $result['rule_cat']    = 'unauthorized';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
  }
  
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*is currently not permitted to.*\n?.*550-relay through this server/",$body,$match)) {
    $result['rule_cat']    = 'unauthorized';
    $result['rule_no']     = '0666';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*Temporary error on maildir delivery.*\n?.*I\'m not going to try again; this message has been in the queue too long./",$body,$match)) {
    $result['rule_cat']    = 'defer';
    $result['rule_no']     = '0163';
    $result['email']       = $match[1];
  }
  ...
  elseif (preg_match ("/^AutoReply message from (\S+@\S+\w)/i",$body,$match)) {
    ...
    //ver el from para ver de que persona es. No hay guid ni referencia al mail enviado
  elseif (preg_match ("/Su mensaje fue recibido. En breve me comunicare con ud. Saludos cordiales/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/This is an automated response/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/Ha llegado tu e\-mail/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }

  elseif (preg_match ("/Muchas gracias.En cuanto me sea posible responder/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/tu mensaje me ha llegado bien/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/SU  MENSAJE HA SIDO RECIBIDO POR EL SERVIDOR/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  ...
  elseif (preg_match ("/Esta es una respuesta automática/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/fuera de la oficina/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/no estaré en mi oficina/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  ...
  elseif (preg_match ("/Estoy de licencia/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/I'm on vacation/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/out of office/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/I will be away until/i",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  
  elseif (preg_match ("/Su mensaje ha sido recibidoi",$body,$match)) {
    $result['rule_cat']    = 'autoreply';
    $result['rule_no']     = '0167';
    $result['email']       = "";
  }
  ...
  //confirmación en respuesta al mail enviado
  //de vacaciones
  ...
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*does not accept[^\r\n]*non-Western/i",$body,$match)) {
    ...
    elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*Sorry, I couldn\'t find any host by that name/i",$body,$match)) {
    $result['rule_cat']    = 'server_unknown';
    $result['rule_no']     = '0441';
    $result['email']       = $match[1];
  }
  
    elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*este dominio nao existe/",$body,$match)) {
    $result['rule_cat']    = 'server_unknown';
    $result['rule_no']     = '0441';
    $result['email']       = $match[1];
  }
  
      elseif (preg_match ("/<(\S+@\S+\w)>.*mail transport unavailable/",$body,$match)) {
    $result['rule_cat']    = 'server_unknown';
    $result['rule_no']     = '0441';
    $result['email']       = $match[1];
  }
  ...
  /*
   * rule: wasn't able to establish an SMTP connection
   * sample:
   * <xxxxx@yourdomain.com>
   ...
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*I wasn\'t able to establish an SMTP connection. \(\#4\.4\.1\)/i",$body,$match)) {
    $result['rule_cat']    = 'server_unknown';
    $result['rule_no']     = '0441';
    $result['email']       = $match[1];
  }

  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*Connected to .* but connection died. \(\#4\.4\.2\)/i",$body,$match)) {
    $result['rule_cat']    = 'server_unknown';
    $result['rule_no']     = '0441';
    $result['email']       = $match[1];
  }  
   ...
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?.*I couldn\'t find a mail exchanger or IP address. \(\#5\.4\.4\)/i",$body,$match)) {
    $result['rule_cat']    = 'server_unknown';
    $result['rule_no']     = '0442';
    $result['email']       = $match[1];
  }
   ...
   * sample:
   * <xxxxx@yourdomain.com>
   * Sorry, I couldn't find any host named yourdomain.com. (#5.1.2)
   */
  elseif (preg_match ("/<(\S+@\S+\w)>.*\n?Sorry, I couldn't find any host named/i",$body,$match)) {
    $result['rule_cat']    = 'server_unknown';
    $result['rule_no']     = '0441';
    $result['email']       = $match[1];
  }
