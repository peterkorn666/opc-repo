...
class BounceMailHandler {
  ...
  /*
   * Determines if blocked bounces will be moved to another mailbox folder
   * NOTE: If true, this will disable delete and perform a move operation instead
   * @var boolean
   */
  public $moveBlocked = false;

  /*
   * Mailbox folder to move blocked bounces to, default is 'blocked'
   * @var string
   */
  public $blockedMailbox = 'INBOX.blocked';

  /*
   * Determines if aurotreply bounces will be moved to another mailbox folder
   * NOTE: If true, this will disable delete and perform a move operation instead
   * @var boolean
   */
  public $moveAutoreply = false;

  /*
   * Mailbox folder to move aurotreply bounces to, default is 'aurotreply'
   * @var string
   */
  public $autoreplyMailbox = 'INBOX.autoreply';
  /*
      ...
      */
      ...
      if ($this->use_fetchstructure) {
        $structure = imap_fetchstructure($this->_mailbox_link,$x);
          ...
          $processed = $this->processBounce($x,$c_total,$bounce_type);
      } else {
      ...
      if ($processed) {
        ...
        if ( ($this->testmode === false) && ($this->disable_delete === false) ) {
          ...
          $c_deleted++;
			...
			if ( $bounce_type=="hard" && $this->moveHard ) {
			...
			if ($bounce_type=="soft" &&  $this->moveSoft ) {
			  // check if the move directory exists, if not create it
			...
			if ($bounce_type=="blocked" &&  $this->moveBlocked ) {
			  // check if the move directory exists, if not create it
			  $this->mailbox_exist($this->blockedMailbox);
			  // move the message
			  @imap_mail_move($this->_mailbox_link, $x, $this->blockedMailbox);
			  $moveFlag[$x] = true;
			  $c_moved++;
			}
			if ($bounce_type=="autoreply" &&  $this->moveAutoreply ) {
			  // check if the move directory exists, if not create it
			  $this->mailbox_exist($this->autoreplyMailbox);
			  // move the message
			  @imap_mail_move($this->_mailbox_link, $x, $this->autoreplyMailbox);
			  $moveFlag[$x] = true;
			  $c_moved++;
			}
		}
   ...
   * @return boolean
   */
  function processBounce($pos,$totalFetched,&$bounce_type) {
    ...
    if ( $bounce_type=="hard" && $this->moveHard && $result['remove'] == 1 ) {
      $remove      = 'moved (hard)';
    } elseif ( $bounce_type=="soft" &&  $this->moveSoft && $result['remove'] == 1 ) {
      $remove      = 'moved (soft)';
    } elseif ( $bounce_type=="autoreply" &&  $this->moveAutoreply && $result['remove'] == 1 ) {
      $remove      = 'moved (autoreply)';
    } elseif ( $bounce_type=="blocked" &&  $this->moveBlocked && $result['remove'] == 1 ) {
      $remove      = 'moved (blocked)';
    ...
    if ($rule_no == '0000') { // internal error      return false;
      ...
      $params = array($pos,$bounce_type,$email,$subject,$fecha,$guid,$remove,$rule_no,$rule_cat,$totalFetched);
