 ...
 * @return boolean
 */
function callbackAction ($msgnum, $bounce_type, $email, $subject, $fecha, $guid, $remove, $rule_no=false, $rule_cat=false, $totalFetched=0) {
  $displayData = prepData($email, $bounce_type, $remove);
  $bounce_type = $displayData[bounce_type];
  $emailName   = $displayData[emailName];
  $emailAddy   = $displayData[emailAddy];
  $remove      = $displayData[remove];
  echo $msgnum . ': '  . $rule_no . ' | '  . $rule_cat . ' | '  . $bounce_type . ' | '  . $remove . ' | ' . $email . ' | '  . $subject . ' | '  . $guid .' | '  . $fecha ."<br />\n";
  echo "<hr>";
  return true;
}
