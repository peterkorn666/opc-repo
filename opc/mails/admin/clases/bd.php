...
function mysqli_prep($value, $link){
...
class BD
{
	...
	function conectar() {
		$this->link = @mysqli_connect($this->host, $this->user, $this->password);//, $this->basedatos
		//no es compatible con todas las versiones de mysql
		//mysqli_query('SET NAMES "'.CHARSET_BD.'"', $this->link);

		$this->errornum= mysqli_errno($this->link);
	...
	//echo "<hr>$query<hr>";
		...
		if ($this->errornum==0) {
			...
			//$nuevo_id = mysqli_insert_id($this->link);
			...
			if ($this->errornum) {
				...
				if(strpos(ltrim(strtoupper($query)),"SELECT")===0) {
					$this->filas = mysqli_num_rows($this->rs);
				} else {
					$this->filas = 0;
				}
				if(strpos(ltrim(strtoupper($query)),"UPDATE")===0 || strpos(ltrim(strtoupper($query)),"DELETE")===0 || strpos(ltrim(strtoupper($query)),"INSERT")===0 || strpos(ltrim(strtoupper($query)),"REPLACE")===0) {
					$this->filasAfectadas = mysqli_affected_rows($this->link);
				} else {
					$this->filasAfectadas = 0;
				}
				if(strpos(ltrim(strtoupper($query)),"INSERT")===0) {
					$this->idInsert = mysqli_insert_id($this->link); //aca es el error
					//$this->idInsert = $nuevo_id;
					//die($this->idInsert);
				}else{
					$this->idInsert = 0;
				}
	...
	function irAFila($numeroFila) {
		@mysqli_data_seek ($this->rs,$numeroFila);
		$this->errornum=mysqli_errno();
		$this->error=mysqli_error();
		return ($this->errornum==0);
	}
	...
	function cantidadCampos() {
		$cantidadCampos=mysqli_num_fields($this->rs);
		$this->errornum=mysqli_errno();
		$this->error=mysqli_error();
	...
	function darCampos() {
		$campos=array();
		$i = 0;
		while ($i < mysqli_num_fields($this->rs)) {
			$f=mysqli_fetch_field($this->rs, $i);
	...
	function condicionTexto($campos,$texto) {
		...
		for ($j=0;$j<$cantidadCampos;$j++) {
				...
				for ($i=0;$i<$cantidadPalabras;$i++){
					if ($palabras[$i]!="") {
						if ($condicionCampo!="")
							$condicionCampo = $condicionCampo . " OR ";//, $this->link
						$condicionCampo = $condicionCampo .  $camposArray[$j] . " LIKE '%" . mysqli_prep($palabras[$i], "a") . "%'";
					}
				}
				if ($condicionCampo!="") {
	...
	function agregarRegistro(&$mensaje,$tabla,$campoClave,$campos) {
		...
		for ($i=0;$i<count($keys);$i++) {
			...
			if (is_null($campos[$keys[$i]])) {
				...
				$valores .=  "'" . mysqli_prep($campos[$keys[$i]], $this->link) . "'";
			}
	    }
	...
	function actualizarRegistro($id, &$mensaje,$tabla,$campoClave,$campos) {
		...
		for ($i=0;$i<count($keys);$i++) {
			...
			if (is_null($campos[$keys[$i]])) {
				...
				$sql .=  str_replace(".","`.`","`" . $keys[$i] . "`") . "=" ."'" . mysqli_prep($campos[$keys[$i]], $this->link) . "'";
			}
		}
