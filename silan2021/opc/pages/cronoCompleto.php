...
$altoAbsoluto = $bannerheight[1]+70;
...
$sqlCrono = $core->query("SELECT c.start_date,c.end_date,c.section_id,c.section_id, s.salaid, s.orden FROM cronograma as c JOIN salas as s ON c.section_id=s.salaid ORDER BY SUBSTRING(c.start_date,1,10), s.orden ASC, c.start_date");
...
{	
	...
	if(substr($crono["start_date"],0,10)=="2020-11-04" && $_SESSION["tipoUsu"]!="1")	
	...
	if($ocultarTrabajos){
		...
		//CRONOGRAMA
			...
			foreach($sql as $row){
				...
				echo "height:" . $alto . "px; ";
				echo "left:".($posX+(8*$leftPlus)).$typeWidth."; ";
				//tipo actividad
				...
				echo "'>";
					...
					if ($aux_bool)
						echo '<div style="font-size:8px; text-align:center;" class="hora_actividad" align="left" >' . "<b>" .$core->helperDate($row["start_date"],11,5) . " - " . $core->helperDate($row["end_date"],11,5) . "</b>";
					if($row["tematica"])
					...
					foreach($sqlC as $rowC):
						echo '<div class="conferencistas" style="font-size:11px">'.$templates->CronoConferencsitas($rowC, array("pais"=>false, "titulo_conf"=>false)).'</div>';
					endforeach;
