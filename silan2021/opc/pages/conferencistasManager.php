...
if(isset($_POST["nombre"]))
{
    ...
    if($archivo_presentacion["tmp_name"]!="")
        $archivo_nombre_presentacion = $core->guardarArchivo($archivo_presentacion,"presentaciones/");
    else
    ...
    $archivo_cv_nombre = "";
    if($archivo_cv["tmp_name"]!="")
        $archivo_cv_nombre = $core->guardarArchivo($archivo_cv,"cv/",base64_decode($_POST["key"])."_".$_POST["nombre"]."_".$_POST["apellido"]);
    else
        $archivo_cv_nombre = $_POST["archivo_cv_viejo"];
	...
	$core->bind('archivo_cv',$archivo_cv_nombre);
	...
	if($core->isAdmin())
	{
		...
		$core->bind('control_inscripcion',($_POST["control_inscripcion"]?1:'0'));
		$core->bind('control_pasaje',($_POST["control_pasaje"]?1:'0'));
		$core->bind('control_hospedaje',($_POST["control_hospedaje"]?1:'0'));
		$core->bind('control_traslado',($_POST["control_traslado"]?1:'0'));
	}
}

if(!empty($_POST["key"]))
{
	$core->bind("key",base64_decode($_POST["key"]));
	if($core->isAdmin())
	{
	$adm = ",hotel=:hotel,
							hotel_in=:hotel_in,
							...
							control_inscripcion=:control_inscripcion,
							control_pasaje=:control_pasaje,
							control_hospedaje=:control_hospedaje,
	...
	$sql = $core->query("UPDATE conferencistas SET 
							...
							WHERE id_conf=:key");
	if(!$core->isAdmin())
		header("Location: /opc/?page=conferencistasManager&status=".$sql."&key=".$_POST["key"]);
...
if($_GET["status"]=="1")
{
	echo "<div class='alert alert-success text-center'>Se guardaron los cambios correctamente.</div>";	
}
?>
<!-- FORM Conferencistas -->

<div id="div-conferencista" style="<?php if(!isset($_GET["key"]) && !isset($_GET["new"])){ echo 'display:none'; }?>">
<form id="confs" action="<?=$config["url_opc"]?>?page=conferencistasManager<?php if($_GET["key"]){echo "&key=" . base64_encode($row["id_conf"]);}?>" method="post" enctype="multipart/form-data" onSubmit="return validar()">
...
<div class="row" style="margin-top:0px !important;<?php if(isset($_GET["new"])){ echo 'display:none'; }?>">
    ...
    <div class="col-md-8" style="background-color:#FFD08A; padding:10px 10px 20px"><br>
        ...
        <div>
        	<input type="checkbox" name="control_inscripcion" value="1" <?php if($row["control_inscripcion"]=="1"){echo "checked";} ?>> Inscripción&nbsp;&nbsp;&nbsp;&nbsp; <input type="checkbox" name="control_pasaje" value="1" <?php if($row["control_pasaje"]=="1"){echo "checked";} ?>> Pasaje&nbsp;&nbsp;&nbsp;&nbsp; <input type="checkbox" name="control_hospedaje" value="1" <?php if($row["control_hospedaje"]=="1"){echo "checked";} ?>> Hospedaje&nbsp;&nbsp;&nbsp;&nbsp; <input type="checkbox" name="control_traslado" value="1" <?php if($row["control_traslado"]=="1"){echo "checked";} ?>> Traslado 
        </div><br>
	...
	<div class="col-md-12">    	
        ...
        <div class="row">
            <div class="col-md-12 text-center">
                ...
                if($row["archivo_presentacion"])
                    echo "<a href='presentaciones/".$row["archivo_presentacion"]."' target='_blank'><i>Ya tiene una conferencia cargada.</i></a> <a href='#' class='eliminar_presentacion'>(Eliminar)</a><br>";
                ?>
                <a href="#" class="uploadFile btn btn-warning input-sm" data-target="presentacion"> Haga click aquí para cargar su presentación</a>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="row">
                        ...
                        ?>
                        <div id="confPhoto" class="col-md-12 well text-center"><img src="conf_fotos/<?=$foto_conf?>" style="max-height: 110px; max-width: 160px"></div>
                    </div>
                ...
                <div class="col-md-9">
                    <div class="col-md-12" style="margin-bottom: 10px">
                        <label><strong>Curriculum abreviado</strong> <em>que se leerá en el momento de su disertación (hasta 100 palabras en cada idioma)</em></label>
        ...
        <div class="row">
            <div class="col-md-12 text-center">
                <?php
                if($row["archivo_cv"])
                    echo "<a href='cv/".$row["archivo_cv"]."' target='_blank'><i>Ya tiene un curriculum cargado.</i></a> <a href='#' class='eliminar_cv'>(Eliminar)</a><br>";
                ?>
               <!-- <a href="#" class="uploadFile btn btn-info input-sm" data-target="archivo_cv"> Haga click aquí para cargar su curriculum</a>
                <input type="file" name="archivo_cv" style="display: none;">
                <input type="hidden" name="archivo_cv_viejo" value="<?=$row["archivo_cv"]?>">-->
            </div>
        </div><br>
 <?php
 if($hotel || $core->isAdmin())
 {
 ?>      
        <!-- Hotel -->
        ...
        <div class="row">
            ...
            <div class="col-xs-2">
                ...
                <input type="text" class="form-control input-sm" name="hotel_traslado_in" value="<?=$row["hotel_traslado_in"]?>">
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-xs-2">
                <label>Traslado</label>
                <input type="text" class="form-control input-sm" name="hotel_traslado_out" value="<?=$row["hotel_traslado_out"]?>">
            </div>
        </div><br><br>
        <?php
		}
	...
	<p align="center">
    	<input type="submit" class="btn btn-primary" value="Guardar" style="width:400px"></p>
		<input type="hidden" id="key" name="key" value="<?=base64_encode($row["id_conf"])?>">
...
<div class="row">
    <div class="col-md-12" style="max-height:400px; overflow:auto;">
    	<table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#000000" class="table">
        	<tr>
				...
				foreach($sql as $a)
				{
					...
					$cantidad_actividades = count($act);
				?>
                 	<tr>
                      ...
                      <a href="<?=$config["url_opc"].'?page='.$_GET["page"].'&key='.base64_encode($a["id_conf"])?>" style="font-weight:bold;">
                        ...
                        <td bgcolor="#FFFFFF"> <a href="<?=$config["url_opc"].'?page='.$_GET["page"].'&key='.base64_encode($a["id_conf"])?>" style="font-weight:bold;"><?=$cantidad_actividades?></a></td>
