...
foreach($sqlDias as $key => $dias)
{
	...
	if($_GET["day"]==$core->helperDate($dias["start_date"],0,10) || (empty($_GET["day"]) && $h==0))
		$chk = "day_selected";			
	echo "<div class='dias_cronograma' style='text-transform: uppercase;'><a href='?page=programaExtendido&day=".$core->helperDate($dias["start_date"],0,10)."' class='$chk'>".utf8_encode(strftime($config["time_format"],strtotime($core->helperDate($dias["start_date"],0,10))))."</a></div>";
	$chk = "";
	++$h;
}
echo "</div>";
...
if($toDay){
	...
	if(!$_SESSION["admin"])
		$where_sala .= ' AND s.visible=1';
}else{
	$where_sala = "";
	if(!$_SESSION["admin"])
		$where_sala .= 'WHERE s.visible=1';
}
