...
class abstracts{
	...
	public function getLastNumTL(){
		$sql = $this->db->query("SELECT * FROM trabajos_libres ORDER BY numero_tl DESC")->first();
	...
	public function getAutoresIDtl($id){
		$sql = $this->db->query("SELECT * FROM trabajos_libres_participantes as tp JOIN personas_trabajos_libres as pl ON tp.ID_participante=pl.ID_Personas WHERE tp.ID_trabajos_libres=? ORDER BY tp.ID ASC", [$id])->results();
		return $sql;
	}
	...
	function guardarArchivoTL($archivo,$trabajo){
		...
		if($archivo["tmp_name"]!=""){
			...
			/*if(
				...
				header("Location: trabajo.php?error=file");
				die();
			}
			if(filesize($archivo["tmp_name"])>20971520)
			{
				header("Location: trabajo.php?error=size");
				die();
			}
			
			$archivo_tl = $trabajo["numero_tl"];
			/* ."_".$trabajo["nombre"]."_".$trabajo["apellido"] */
			$archivo_tl = $this->reemplazarNombreArchivo($archivo_tl.".".$extension);
			@unlink("../tl/$archivo_tl");
			if(!copy($archivo["tmp_name"], "../tl/$archivo_tl")){
	...
	private function saveTL(){	
		...
		if($_SESSION["abstract"]["tipo_tl"] == 1){
			$resumen_tl4 = $_SESSION["abstract"]["resumen_tl4"];
		}else{
			$resumen_tl4 = NULL;
		}
		...
		$data = array(
			...
			"premio"=>$_SESSION["abstract"]["premio"],
			"resumen"=>$_SESSION["abstract"]["resumen_tl"],
			"resumen2"=>$_SESSION["abstract"]["resumen_tl2"],
			"resumen3"=>$_SESSION["abstract"]["resumen_tl3"],
			"resumen4"=>$resumen_tl4,
			"resumen5"=>$_SESSION["abstract"]["resumen_tl5"],
			"resumen6"=>$_SESSION["abstract"]["resumen_tl6"],
	...
	public function todoTL($solo_completo = 0){
        try{
            //Simulate Queries
            $this->db->beginTransaction();
			if($solo_completo){
				$merge = $_SESSION["abstract"];
				$merge1 = array("nombre" => $_SESSION["abstract"]["nombre_0"], "apellido" => $_SESSION["abstract"]["apellido_0"]);
				$trabajo = array_merge($merge, $merge1);
				$archivo_tl = $this->guardarArchivoTL($_FILES["archivo_tl"], $trabajo);
				if($archivo_tl){
					$r = $this->db->update("trabajos_libres", "id_trabajo=".$_SESSION["abstract"]["id_tl"], ["archivo_tl" => $archivo_tl]);
				}else{
					throw Exception("Debe seleccionar un archivo.");
				}
			}else{
            	$tl = $this->saveTL();
			...
			$this->db->commit();
			return array("success"=>1,"numero_tl"=>$tl["numero_tl"], "archivo_tl" => $archivo_tl);
		}catch(Exception $e){
			//Rolback all changes.
            if(\Config::get('debug'))
