          ...
          <tr>
            ...
            <td>
              <select name="estadoTL">
                ...
                <option value="Aceptado" <? if($estadoTL=="Aceptado"){echo "selected";} ?>>ACEPTADO</option>
                <option value="Aceptado con modificaciones" <? if($estadoTL=="Aceptado con modificaciones"){echo "selected";} ?>>ACEPTADO CON MODIFICACIONES</option>
                <option value="No aceptado" <? if($estadoTL=="No aceptado"){echo "selected";} ?>>NO ACEPTADO</option>
                </select>
              </td>
            </tr>
          <tr>
...
no evaluados</td>
    ...
    <?
	     ...
	     /*if($_SESSION["nivel"]!=1){
		  ...
		  if($_SESSION["nivel"]!=1){
				/*$sqlTrabajosEvaluador = "SELECT numero_tl FROM evaluaciones WHERE idEvaluador='".$_SESSION["idEvaluador"]."' ORDER BY numero_tl";
				$resultEvaluador = $con->query($sqlTrabajosEvaluador);
				if($resultEvaluador){
					$arrayTrabajosEvaluadorSQL = "(";
					while($rowTrabajosEvaluador = $resultEvaluador->fetch_array()){
						$arrayTrabajosEvaluadorSQL .= $rowTrabajosEvaluador["numero_tl"].", ";
					}
					$arrayTrabajosEvaluadorSQL = substr($arrayTrabajosEvaluadorSQL,0,-2);
					$arrayTrabajosEvaluadorSQL .= ")";
				}else{
					$arrayTrabajosEvaluadorSQL = "('')";
				}
				$sqlT = "SELECT * FROM evaluaciones as e JOIN trabajos_libres as tr ON e.numero_tl=tr.numero_tl JOIN evaluadores as ev ON e.idEvaluador=ev.id WHERE e.numero_tl IN ".$arrayTrabajosEvaluadorSQL." ORDER BY area_tl, e.numero_tl;";*/
				$sqlT = "SELECT * FROM evaluaciones as ev JOIN trabajos_libres as tr ON ev.numero_tl=tr.numero_tl WHERE ev.idEvaluador ='".$_SESSION["idEvaluador"]."' ORDER BY tr.area_tl, ev.numero_tl;";
		  }else{
			  	//if($filtro){
					$sqlT = "SELECT * FROM evaluaciones as e JOIN trabajos_libres as t ON e.numero_tl=t.numero_tl JOIN evaluadores as ev ON e.idEvaluador=ev.id WHERE 1 $filtro $orden";
				/*}else{
					$sqlT = "";
				}*/
		  }
 ...
 Revisado</td>
		...
		$areaV = $row["area_tl"];
		 ?>
          ...
          <?
			if($_SESSION["nivel"]==1){
		  ?>
		    <td align="center" bgcolor="#F4F4F4" ><input type="checkbox" name="tl[]" value="<?=$row["id_trabajo"]?>"></td>
          <?
			}
		  ?>
        ...
        <?
				...
				if($_SESSION["nivel"]!=1){
					if($_SESSION["idEvaluador"] == $row["idEvaluador"]) {
			?>
      ...
      <?
						...
						echo "<td align='center' bgcolor='#F4F4F4'></td>";
					}
...
function moverTrabajos(){
 document.form1.method = "POST";
 document.form1.action = "../moverTL.php?c=1";
 document.form1.submit();
}
