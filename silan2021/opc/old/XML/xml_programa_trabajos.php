...
include("../conexion.php");
	...
	//Programa
	$sql = "SELECT * FROM congreso WHERE Dia_orden=3 ORDER by Dia_orden,Hora_inicio,Sala_orden,Orden_aparicion;";
...
while ($row = mysql_fetch_array($rs)){
	// &&  &&  && 
	//($row["Tipo_de_actividad"]!="Presentación de Videos")
	//$row["Tipo_de_actividad"]!="Presentación de E-Poster")
	//($row["Tipo_de_actividad"]!="Presentación de trabajos Orales")
if(($row["Tipo_de_actividad"]=="Sesión Científica SIUP")){}else{
...
/**/
	...
	//if($hora_anterior != $row["Hora_inicio"]){
		...
		//if($row["Hora_inicio"]=="14:00:00" or $row["Hora_inicio"]=="13:00:00"){
			//echo  "<br>" . nodo("dia") . $row["Dia"] . nodo("/dia") . "### " ;
			echo   "<br>".nodo("Hora") . substr($row["Hora_inicio"], 0, -3). " a " . substr($row["Hora_fin"],0 , -3) . nodo("/Hora");
			echo   "###".nodo("Titulo_de_actividad") . $row["Titulo_de_actividad"]. nodo("/Titulo_de_actividad");
	//	}
	...
	*/	
	...
	if($row_t["archivo_tl"]!=""){
		...
		//
		echo nodo("hora_tl").remplazar(substr($row_t["Hora_inicio"],0,-3)). nodo("/hora_tl")."@@@..@".nodo("numero_tl").remplazar($row_t["numero_tl"]). nodo("/numero_tl")."@@@..@".nodo("titulo").remplazar($row_t["titulo_tl"]). nodo("/titulo"). "<br>";
				...
				*/			
				...
				$sql2 ="SELECT ID_participante, lee FROM trabajos_libres_participantes WHERE ID_trabajos_libres = " .  $row_t["ID"] . " ORDER BY ID ASC;";
					...
					//	}
						...
						if($arrayPersonas[$i][3]!=""){
							$paisPresentador = " ".nodo("paisAutor")."(".$arrayPersonas[$i][3].")".nodo("/paisAutor");
						}
						echo $arrayPersonas[$i][1]. ", " . $arrayPersonas[$i][2].$paisPresentador;
