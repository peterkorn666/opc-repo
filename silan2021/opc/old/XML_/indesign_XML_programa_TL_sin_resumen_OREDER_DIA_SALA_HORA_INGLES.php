...
include("../conexion.php");
	//SUCS
	//$sql = "SELECT * FROM congreso WHERE Tipo_de_actividad<>'Posters' ORDER by Sala_orden,Dia_orden,Hora_inicio;";
	
	//Programa
	//$sql = "SELECT * FROM congreso ORDER by Dia_orden,Hora_inicio,Sala_orden;";
...
if( ($row["Sala"]!="Sala Florida - SUCS") ){}else{
	...
	echo  "<br>" . nodo("dia_ing") . $rowDias["Dia_ing"] . nodo("/dia_ing") ;
	...
	if($hora_anterior != $row["Hora_inicio"]){
		/*if($casillero_anterior1 != $row["Casillero"]){	
			...
			echo   "@@@". nodo("tipo_actividad_ing") . remplazar($row_act["Tipo_de_actividad_ing"]) . nodo("/tipo_actividad_ing");
}
...
/**/
	...
	*/	
		...
		if($row["Titulo_de_actividad_ing"]!=""){
			echo  "<br>" . nodo("titulo_actividad_ing") . remplazar($row["Titulo_de_actividad_ing"]) . nodo("/titulo_actividad_ing");
		}

		$casillero_anterior = $row["Casillero"];
	}

	if($tit_trabajo_anterior != $row["Titulo_de_trabajo_ing"] && $row["Titulo_de_trabajo_ing"]!=""){
		echo  "<br>" .  nodo("titulo_trabajo_ing") . remplazar($row["Titulo_de_trabajo_ing"])  .  nodo("/titulo_trabajo_ing");
		$tit_trabajo_anterior = $row["Titulo_de_trabajo_ing"];
	}
	...
	if($row["Trabajo_libre"]==1){
		...
		$sql_t ="SELECT * FROM trabajos_libres WHERE ID_casillero= '" . $row["Casillero"] . "' AND estado<>3 ORDER BY numero_tl ASC;";
		$rs_t = mysql_query($sql_t,$con);
		while ($row_t = mysql_fetch_array($rs_t)){
	...
	echo  "". nodo("numero_Area")  .$numeral.remplazar($row_t["numero_tl"]). nodo("/numero_Area");
