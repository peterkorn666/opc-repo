...
function fieldEnter(campo,evt,idfld) {
	...
	/*AGREGADO PARA PONER UN CARGANDO*/
	MostrarDiv('DivGuardar');
	/**/
		nt = remotos.enviar(urlBase + "?fieldname=" +encodeURI(elem.id)+ "&content="+encodeURI(campo.value)+"&"+formVars,"");
		//remove glow
		noLight(elem);
		elem.innerHTML = nt;
		//AGREGADO PARA LLENAR EL CONTROL DE TEXTO
		nombretext = String(elem.id);
		campoText = nombretext.substring(4,nombretext.length); 
		//PONGO LO QUE ACABO DE ESCRIBIR EN EL CAMPO DE TEXTO POR SI APRETA EL BTN
		document.getElementById( campoText ).value=campo.value;
		///
		/*AGREGADO PARA PONER UN CARGANDO*/
		setInterval("OcultarDiv('DivGuardar')",900);
		/**/
		changing = false;
		return false;
	} else {
...
function fieldBlur(campo,idfld) {
	...
	if (campo.value!="") {
		...
		//AGREGADO PARA LLENAR EL CONTROL DE TEXTO
		nombretext = String(elem.id);
		//PONER EL ID DEL SPAN SPAN + NOMBRE DEL CONTROL REAL INPUT
		campoText = nombretext.substring(4,nombretext.length); 
		//PONGO LO QUE ACABO DE ESCRIBIR EN EL CAMPO DE TEXTO POR SI APRETA EL BTN
		document.getElementById( campoText ).value=campo.value;
		///
...
/*	if(campo.value == "Ingrese aquí el Titulo de la actividad"){
		campo.value = "";
	}
	*/
...
function editbox_init(){
	...
	for (var i=0; i<spans.length; i++){
		var spn = spans[i];
			...
			spn.title = "Haga click para editar el contenidos.";	
       		}
