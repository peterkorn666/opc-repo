...
function reemplazarAcute($cuerpo){
...
while($row= $rs->fetch_array()){
	...
	while ($rowEv = $sqlEv->fetch_array()) {
		if ($rowEv["comentarios"] != "") {
			$comentario_db = reemplazarAcute($rowEv["comentarios"]);; //$comentario_db = reemplazarVocales($rowEv["comentarios"]);
			if ($comentarios_evaluador != "") {
				$comentarios_evaluador = $comentarios_evaluador . "<br><br>Evaluador " . $k . ": <br>" . $comentario_db;
				$k++;
			} else {
				$comentarios_evaluador = "<br>A continuaci&oacute;n, le detallamos las correcciones que deber&aacute; tener en cuenta:<br><br>Evaluador 1: <br>" . $comentario_db;
				$k++;
			}
		}
	}
	...
	//Titulo actividad
	$sqlCronogramaTrabajo = $con->query("SELECT id_crono, titulo_actividad FROM cronograma WHERE id_crono='".$row["id_crono"]."'");
	$rowCronogramaTrabajo = $sqlCronogramaTrabajo->fetch_array();
	$titulo_actividad = $rowCronogramaTrabajo["titulo_actividad"];
...
if($_SESSION["chkMostrarUbicacion"]!="1"){
	...
	if($casilla!=0){
		...
		while($rowU = $sqlU->fetch_array()){
			...
			$ubicacion .= "<div style='text-align:right'>";
			...
			$ubicacion .= "<strong>".$rowU["titulo_tl"]." &nbsp;&nbsp; (#".$rowU["numero_tl"].")</strong><br><br>";
			//Autores
		 ...
		 '.$tituloTl.'
		  ...
		  <div align="justify"> <font style="font-family: Times New Roman, Times, serif; font-size: 12px;"> 
			...
			/*if($resumen!=""){
				...
				if($resumen){
					$muestroPantalla .= '
						<div><strong>INTRODUCCIÓN:</strong><br />
...
$link_editar_aqui = "<a href='".$paginaAbstract."login.php?id=".base64_encode($id_trabajo)."'>aquí</a>";


$cuerpoMail = str_replace("<:dirBanner>", $dirBanner , $cartaEstandarTrabajos);
$cuerpoMail = str_replace("<:congreso>", utf8_encode($congreso), $cuerpoMail);
...
$cuerpoMail = str_replace("<:link_abstract>", $paginaAbstract, $cuerpoMail);
$cuerpoMail = str_replace("<:link_trabajo_completo>", "", $cuerpoMail);
$cuerpoMail = str_replace("<:link_editar_trabajo_aqui>", $link_editar_aqui, $cuerpoMail);
$cuerpoMail = str_replace("<:titulo_actividad>", $titulo_actividad, $cuerpoMail);
...
//$cuerpoMail = utf8_decode($cuerpoMail);
...
$mailOBJ->addReplyTo($mail_congreso, "Contacto - ".utf8_encode($congreso));
...
$mailOBJ->FromName 		= utf8_encode($congreso);
