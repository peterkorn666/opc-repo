...
$cartas_predefinidas = $cartas_instancia->getCartasInscriptos();
$inscriptos_con_mail = $inscripto_instancia->getInscriptosConMail();

$costos_inscripcion = $inscripto_instancia->getCostosInscripcion();
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    <title>Inscriptos - Envio de mail - <?=$config["nombre_congreso"]?></title>
    ...
    <div class="container">
        ...
        <form id="formCarta" action="process.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="col-md-12 datos_envio">
                ...
                <div class="row">
                    <div class="col-md-8">
                        <input type="checkbox" name="enviar_a_personas" value="1"> Enviar el mail correspondiente a cada inscripto seleccionado
                    </div>
                </div><br>
                <div class="row">
                    <div class="col-md-12">
            ...
            <div class="col-md-12 datos_listado">
                <div class="row div_info">
                    <div class="col-md-12">
                        <strong>Inscriptos</strong><br>
                        Hay [<strong><?=count($inscriptos_con_mail)?></strong>] inscriptos que tienen e-mail de contacto.
                    </div>
                </div>
                <div class="row div_filtros">
                    <div class="col-md-6">
                        <label>Filtro costos inscripción:</label>
                        <select name="filtro_costos_inscripcion" class="form-control">
                            <option value=""></option>
                            <?php
                            foreach($costos_inscripcion as $costo_inscripcion){
                                ?>
                                <option value="<?=$costo_inscripcion['id']?>"><?=$inscripto_instancia->formatedPrecio($costo_inscripcion["precio"])?> - <?=$costo_inscripcion["nombre_es"]?></option>
                                <?php
                            }
                            ?>
                        </select>
                    ...
                    <div class="col-md-6">
                        <label>Filtro por idioma:</label>
                        <select name="filtro_idioma" class="form-control">
                            <option value=""></option>
                ...
                <div id="listado">
                <?php
                foreach($inscriptos_con_mail as $inscripto){
                    $pais_inscripto = $auxiliar_instancia->getPaisByID($inscripto["pais"]);
                    ?>
                    <div class="row">
                        <div class="col-md-12">
                            <input type="checkbox" id="personas[]" name="personas[]" value="<?=$inscripto["id"]?>" class="checkbox_personas" data-costo_inscripcion="<?=$inscripto["costos_inscripcion"]?>" data-lang="<?=$inscripto["lang"]?>"> <?=$inscripto["apellido"]?>, <?=$inscripto["nombre"]?> (<?=$pais_inscripto["Pais"]?>) [<?=$inscripto["email"]?>]
                        </div>
                    </div>
                    <?php
                }
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#formCarta").submit(function(){
            ...
            if(checkboxs_personas.length === 0){
                ...
                alert("Debe seleccionar al menos un inscripto a enviar el mail.");
                return false;
            }
        });
    });
                    ...
                    "<div class='col-md-12'>" +
                        "<div class='row'>" +
                            "<div class='col-md-12'>" +
                                ...
                                "<input type='checkbox' name='asunto_numero_inscripto' value='1'> Agregar número de inscripto - <span class='asunto_muestra'></span> [No. inscripto]<br>" +
                    ...
                    "<div class='col-md-12'>" +
                        "<div class='row'>" +
                            "<div class='col-md-12'>" +
                                ...
                                "<input type='checkbox' name='asunto_numero_inscripto' value='1'> Agregar número de inscripto - "+asunto+" [No. inscripto]<br>" +
    ...
    function marcarTodos(){
        $(".checkbox_personas").each(function(){
            var se_marca = true;
            var filtro_costos_inscripcion = $("select[name='filtro_costos_inscripcion'] option:selected");
            if(filtro_costos_inscripcion.val() !== undefined && filtro_costos_inscripcion.val() !== ""){

                var costo_inscripcion = $(this).data('costo_inscripcion');
                if(parseInt(filtro_costos_inscripcion.val()) !== parseInt(costo_inscripcion)){
                    se_marca = false;
                }
            }
            if(se_marca === true){
                var filtro_idioma = $("select[name='filtro_idioma'] option:selected");
                if(filtro_idioma.val() !== undefined && filtro_idioma.val() !== ""){

                    var idioma = $(this).data('lang');
                    if(filtro_idioma.val() !== idioma){
                        se_marca = false;
                    }
                }
            }
