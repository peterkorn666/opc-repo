...
if(count($_SESSION["envio_mail"]["conferencistas"]["personas"]) == 0){

    unset($_SESSION["envio_mail"]["conferencistas"]);
    $div_fin = "<br><div style=\'text-align: center;\'>------------- El envio a finalizado correctamente -------------<br /><br /><a href=\'index.php\'>[ Volver al envio de e-mail para conferencistas]</a></div><br>";
    echo "<script>\n";
    ...
    $persona = array_shift($_SESSION["envio_mail"]["conferencistas"]["personas"]);
    ...
    $txt_a_devolver = "[".$persona["contador_mail"]." de ".$_SESSION["envio_mail"]["conferencistas"]["cantidad_a_enviar"]."] Se ha enviado el mail de ".$persona["persona_nombre"]." ".$persona["persona_apellido"]." (".$persona["persona_email"].") a ";
    $txt_error = "";

    foreach($persona["mails_a_enviar"] as $cualMail){
        if($_SESSION["envio_mail"]["conferencistas"]["adjunto"]){
            $mailOBJ->AddAttachment("../adjuntos/conferencistas/" . $_SESSION["envio_mail"]["conferencistas"]["adjunto"], $_SESSION["envio_mail"]["conferencistas"]["adjunto"]);
        }
        ...
        if(!$mailOBJ->Send()){
            ...
            $campos_carta_enviada_personas = array(
                "carta_enviada_id" => $_SESSION["envio_mail"]["conferencistas"]["carta_enviada_id"],
                "conferencista_id" => $persona["conferencista_id"]
            );
